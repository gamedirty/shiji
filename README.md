...
数据以 JSON 存在本地（`shared_preferences`，单一状态键整状态提交、避免应用层多键中间态，带 schemaVersion 与加载校验；不等同于数据库级事务），无需数据库与网络权限；删除应用即清空。所有写命令串行执行，写盘成功才更新内存，失败会提示且不改动现有数据。首次启动只预置食材与组合餐示例，**不伪造任何饮食记录**。

「食材库 → 备份/恢复」可把全部数据导出为 JSON 文本（剪贴板），或从剪贴板备份恢复。

## 后续可扩展

- 计划复用：套用日模板 / 重复到指定日期（「复制前一天」已上线）
- 体重记录与趋势
- 数据导出为文件 / 云同步
- 食材条码扫描、自定义分类标签、生熟状态
- 饮水记录
